...
2. `scripts/get_latest_discharge.py`
   - 用于获取各站点最近一次测流数据。
   - 默认查询最近 2 小时。
   - 输出内容包括各站点最近一次算法测量水位、平均流速、流量、时间、状态等。
   - 适合问题：
     - "最近一小时/两小时情况如何？"
     - “各站点最新测流数据是多少？”
     - “最近一次测流结果”
...
综合水雨情摘要脚本：
E:\Code Program\LLM-Graph\skills\wuxiwater-skill\scripts\get_one_day_data.py
最近一次测流数据脚本：
E:\Code Program\LLM-Graph\skills\wuxiwater-skill\scripts\get_latest_discharge.py
